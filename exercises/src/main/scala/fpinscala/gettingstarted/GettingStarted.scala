...
object MyModule {
  ...
  def fib(n: Int): Long = {
    @tailrec
    def fibLocal(n: Int, result: List[Long]): List[Long] = n match {
      case 0 => result
      case n => fibLocal(n - 1, result.head + result.tail.head :: result)
    }
    n match {
      case 0 => 0
      case 1 => 1
      case n => fibLocal(n, List(0, 1)).head
    }
  }
...
object PolymorphicFunctions {
  ...
  def main(args: Array[String]): Unit = {
    println(isSorted(Array(0,1,2,1,3), (a:Int, b:Int) => a < b))
  }
  ...
  def isSorted[A](as: Array[A], gt: (A,A) => Boolean): Boolean = {
    def check(index: Int): Boolean = {
      ...
      else if(gt(as(index), as(index+1))) {
        check(index + 1)
      } else {
        false
      }
